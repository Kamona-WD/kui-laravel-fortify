...
const twoFactorEnabled = computed(() => {
    return ! enabling.value && usePage().props.value.auth.user.two_factor_enabled
})
...
const enableTwoFactorAuthentication = () => {
    ...
    Inertia.post('/user/two-factor-authentication', {}, {
        ...
        onSuccess: () => Promise.all([
            ...
            showRecoveryCodes(),
            successToast({
                text: trans(`2FA enabled successfully! :)`)
            })
        ]),
...
const regenerateRecoveryCodes = () => {
    axios.post('/user/two-factor-recovery-codes')
            .then(response => {
                showRecoveryCodes()
                successToast({
                    text: trans(`RecoveryCodes regenerated successfully! :)`)
                })
            })
}
...
const disableTwoFactorAuthentication = () => {
    ...
    Inertia.delete('/user/two-factor-authentication', {
                ...
                onSuccess: () => {
                    ...
                    successToast({
                        text: trans(`2FA disabled successfully! :)`)
                    })
                },
            })
}
